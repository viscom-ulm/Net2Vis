...
def check_exists(identifier):
  ...
  if not os.path.exists('models'):
    os.mkdir('models')
  if not os.path.exists(os.path.join('models', identifier)):
    os.mkdir(os.path.join('models', identifier))
    os.mkdir(os.path.join('models', identifier, 'visualizations'))
    copyfile(os.path.join('default', 'model_current.py'),
             os.path.join('models', identifier, 'model_current.py'))
    copyfile(os.path.join('default', 'layer_types_current.json'),
             os.path.join('models', identifier, 'layer_types_current.json'))
    copyfile(os.path.join('default', 'preferences.json'),
             os.path.join('models', identifier, 'preferences.json'))
    copyfile(os.path.join('default', 'groups.json'),
             os.path.join('models', identifier, 'groups.json'))
    copyfile(os.path.join('default', 'legend_preferences.json'),
             os.path.join('models', identifier, 'legend_preferences.json'))
...
def backup_download_content(identifier):
  ...
  base_location = os.path.join('models', identifier)
  save_location = os.path.join(base_location,
...
def get_code(identifier):
  ...
  """
  check_exists(identifier)
  with open(os.path.join('models', identifier, 'model_current.py'),
            'r') as myfile:
...
def get_layer_types(identifier):
  ...
  """
  check_exists(identifier)
  with open(os.path.join('models', identifier, 'layer_types_current.json'),
            'r') as myfile:
...
def update_layer_types(identifier):
  ...
  check_exists(identifier)
  content = request.data
  file = open(os.path.join('models', identifier, 'layer_types_current.json'),
              'w')
  file.write(content.decode("utf-8"))
  return content, ok_status, text_type
...
def get_preferences(identifier):
  ...
  """
  check_exists(identifier)
  with open(os.path.join('models', identifier, 'preferences.json'),
            'r') as myfile:
...
def update_preferences(identifier):
  ...
  check_exists(identifier)
  content = request.data
  file = open(os.path.join('models', identifier, 'preferences.json'),
              'w')
  file.write(content.decode("utf-8"))
  return content, ok_status, text_type
...
def get_groups(identifier):
  ...
  """
  check_exists(identifier)
  with open(os.path.join('models', identifier, 'groups.json'), 'r') as myfile:
    groups = myfile.read()
...
def get_legend_preferences(identifier):
  ...
  """
  check_exists(identifier)
  with open(os.path.join('models', identifier, 'legend_preferences.json'),
            'r') as myfile:
    preferences = myfile.read()
    return preferences, ok_status, text_type
...
def update_legend_preferences(identifier):
  ...
  check_exists(identifier)
  content = request.data
  file = open(os.path.join('models', identifier, 'legend_preferences.json'),
              'w')
  file.write(content.decode("utf-8"))
  return content, ok_status, text_type
...
def process_vis(identifier):
  ...
  base_path = os.path.join('models', identifier, 'visualizations')
  # Save and convert the svg files into PDFs
  save_and_convert_svgs(base_path, json_content)
  # Create a Zip File
  zipf = zipfile.ZipFile(os.path.join('models', identifier,
                                      'visualizations.zip'), 'w',
  ...
  return send_file(os.path.join('models', identifier, 'visualizations.zip'),
                   mimetype='zip',
