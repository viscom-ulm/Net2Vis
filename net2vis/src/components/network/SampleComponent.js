...
const SampleComponent = ({extent, x, edge, layer_max_height, strokeWidth}) => {
  ...
  const transform = `translate(${x}, ${y_pos + (layer_max_height / 2.0) - (extent / 2.0)})`; // Manipulate the position of the graph
  const style = {
    stroke: 'lightgrey',
