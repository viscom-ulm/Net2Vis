...
export function buildGraphFromNetwork(network, layer_extreme_dimensions, preferences) {
  ...
  graph.setGraph({ranker: 'network-simplex', rankdir: 'LR', ranksep: (preferences.layers_spacing_horizontal.value + preferences.stroke_width.value), nodesep: preferences.layers_spacing_vertical.value}); // Set Graph Properties
  ...
  for (var i in network.layers) { // Add all Layers to the Graph
    ...
    if (layer.properties.dimensions.in.length > 1 && layer.properties.dimensions.out.length > 1) {
      ...
      height = height > preferences.layer_display_max_height.value ? preferences.layer_display_max_height.value : height; // Cap the height when something goes wrong
