...
export function sortGroups(groups, legendItems) {
  for (var i = 0; i < groups.length - 1; i++) { // Go over all groups
    if (groupMustGoToEnd(groups, i)) { // Check if the group should be moved to the end
...
function groupMustGoToEnd(groups, i) {
  for (var j = i + 1; j < groups.length; j++) { // For all groups after this group
    for (var k in groups[i].layers) { // For all the layers of the original Group
      if (groups[i].layers[k].name === groups[j].name) { // If the layer is the currently inspected Group
...
function moveGroupToEnd(groups, i, legendItems) {
  for (var j in legendItems) { // For all Legend Items
    if (j === groups[i].name) { // If it is the searched one
