...
function findGroupDependenciesByName(selectedItem, groups) {
  ...
  for (var i = 0; i < groups.length; i++) { // For all Groups
    if (groups[i].name === selectedItem) { // If this is the given Group
      deps.push(i); // Add it to the Dependencies
      for (var j = 0; j < groups.length; j++) { // For all Groups
        for (var k = 0; k < groups[j].layers.length; k++) { // For all of ther Layers
          if (groups[i].name === groups[j].layers[k].name) { // If a layer has the same name as the Group that was given
            var recursion = findGroupDependenciesByName(groups[j].name, groups); // Recursively resolve their dependencies
            for (var l in recursion) { // For all recursive deps
...
function findGroupDependentsByName(selectedItem, groups) {
  ...
  for (var i = 0; i < groups.length; i++) { // For all Groups
    if (groups[i].name === selectedItem) { // If this is the current Group
      ...
      for (var j = 0; j < groups[i].layers.length; j++) { // For all Layers of this Group
        for (var k = 0; k < groups.length; k++) { // For all Groups
          if (groups[k].name === groups[i].layers[j].name) { // If a Group has the same name as one of the Layers of the current Group
